...
impl Editor {
    ...
    pub fn insert_task_on_current_line(&mut self) -> bool {
        ...
        let (insert_col, text) = match ListPrefix::detect(line) {
            ...
            None if crate::core::markdown::heading(line).is_some() || trimmed.starts_with('>') || trimmed.starts_with('|') || trimmed.starts_with("<details") || trimmed.starts_with("<summary") || crate::core::markdown::is_display_math_delimiter(line) => {
                return false;
            }
