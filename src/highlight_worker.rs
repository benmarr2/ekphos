...
use crate::editor::{EditorSnapshot, HighlightRange, HighlightType, WikiLinkRange};
...
fn detect_header_fast(line: &str) -> Option<usize> {
    crate::core::markdown::heading(line.trim_start()).map(|_| line.chars().count())
}
...
fn highlight_math_fast(row: usize, line: &str, colors: &HighlightColors, highlights: &mut Vec<HighlightRange>) {
    if crate::core::markdown::display_math_body(line).is_some() {
    ...
    crate::core::markdown::visit_inline_math(line, |expression| {
...
fn highlight_links_fast(row: usize, line: &str, colors: &HighlightColors, highlights: &mut Vec<HighlightRange>) {
    ...
    while let Some(relative_start) = line[cursor..].find('[') {
        ...
        let Some(link) = crate::core::markdown::markdown_link_at(line, start) else {
            cursor = start + 1;
            continue;
        };
...
fn compute_snapshot_wiki_links(snapshot: &EditorSnapshot, frontmatter_end: Option<usize>, rows: std::ops::Range<usize>, mut is_cancelled: impl FnMut() -> bool) -> Option<Vec<WikiLinkRange>> {
    ...
    for (row, line) in snapshot.iter_lines().take(rows.end).enumerate() {
        ...
        match crate::core::markdown::fence_marker(line) {
            Some(crate::core::markdown::FenceMarker::Backtick) => {
                in_code_block = !in_code_block;
                continue;
            }
...
mod tests {
    ...
    fn test_detect_frontmatter() {
        ...
        assert_eq!(crate::core::markdown::frontmatter_end_in_lines(lines), Some(2));
        let lines_no_fm = vec!["# No frontmatter", "Content"];
        assert_eq!(crate::core::markdown::frontmatter_end_in_lines(lines_no_fm), None);
    }
