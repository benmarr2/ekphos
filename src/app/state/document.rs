...
fn parse_text_links(text: &str, wiki_exists: &dyn Fn(&str) -> bool) -> Vec<LinkInfo> {
    ...
    let mut links: Vec<LinkInfo> = App::parse_markdown_links_in_text(text)
        ...
        .collect();
    links.extend(crate::core::markdown::wiki_links(text).into_iter().map(|link| {
...
impl App {
    ...
    pub fn heading_level(line: &str) -> Option<usize> {
        crate::core::markdown::heading(line).filter(|heading| heading.level <= 3 && line[heading.level..].starts_with(' ')).map(|heading| heading.level)
    }
    ...
    pub(super) fn bracket_link_at(s: &str, at: usize) -> Option<(String, String, usize, usize)> {
        let link = crate::core::markdown::markdown_link_at(s, at)?;
        if link.kind != crate::core::markdown::MarkdownLinkKind::Link || link.destination.is_empty() {
            return None;
        }
    ...
    pub fn toggle_task_at(&mut self, index: usize) {
        ...
        let Some(updated) = crate::tasks::set_checked(line, !checked, self.dependencies.clock.today()) else {
            return;
        };
