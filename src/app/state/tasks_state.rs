...
pub use crate::tasks::{DueState, Priority, Task as TaskItem};
...
impl TaskViewState {
    ...
    pub fn apply_filters(&mut self, today: chrono::NaiveDate) {
        crate::tasks::sort_tasks(&mut self.tasks);
...
pub(crate) fn filter_task_indices(tasks: &[Task], status: TaskStatusFilter, due: TaskDueFilter, priority: TaskPriorityFilter, query: &str, today: chrono::NaiveDate) -> Vec<usize> {
    tasks
        ...
        .filter(|(_, task)| match priority {
            ...
            TaskPriorityFilter::High => task.priority == Some(crate::tasks::Priority::High),
            TaskPriorityFilter::Medium => task.priority == Some(crate::tasks::Priority::Medium),
            TaskPriorityFilter::Low => task.priority == Some(crate::tasks::Priority::Low),
        })
...
fn collect_all_tasks(root: &std::path::Path, notes: &[Note], request_generation: u64, generation: &AtomicU64) -> Option<Vec<Task>> {
    ...
    for note in notes {
        ...
        tasks.extend(crate::tasks::collect_tasks(&body, note.id, &relative, &note.title));
    }
...
impl App {
    ...
    pub fn toggle_task_from_view(&mut self) {
        ...
        let Some(range) = crate::tasks::locate_task_line(&body, task.source_line, &task.text) else {
            self.show_error_toast("Task moved since the last scan, refreshing");
            self.tasks.dirty = true;
            return;
        };
        let Some(updated) = crate::tasks::set_checked(&body[range.clone()], !task.checked, today) else {
            self.tasks.dirty = true;
            return;
        };
...
mod tests {
    ...
    fn task(text: &str, checked: bool, due: Option<NaiveDate>, priority: Option<crate::tasks::Priority>) -> Task {
    ...
    fn filters_narrow_by_status_due_priority_and_query() {
        let tasks = vec![task("overdue", false, Some(NaiveDate::from_ymd_opt(2026, 1, 1).unwrap()), Some(crate::tasks::Priority::High)), task("today", false, Some(today()), None), task("done today", true, Some(today()), None), task("plain", false, None, None)];
