...
impl App {
    ...
    pub fn open_or_create_journal(&mut self) {
        ...
        let entry = match crate::vault::journal::open_or_create_entry(&notes_dir, &self.state.config.journal_dir, date) {
            Ok(entry) => entry,
            Err(error) => {
        ...
        if self.select_note_by_path(&entry.path) {
            let action = match entry.action {
                crate::vault::journal::JournalEntryAction::Created => "Created",
                crate::vault::journal::JournalEntryAction::Opened => "Opened",
            };
