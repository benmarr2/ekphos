...
impl App {
    ...
    pub(super) fn confined_vault_relative_path(&self, relative: &str) -> Result<PathBuf, String> {
        let relative = crate::core::VaultPath::new(relative).map_err(|_| "The path must stay inside the vault".to_string())?;
    ...
    pub(super) fn directory_has_notes(path: &std::path::Path) -> bool {
        crate::vault::contains_supported_document(path)
    }
    ...
    pub fn load_notes_from_dir(&mut self) {
        ...
        let previous_fingerprints: HashMap<NoteId, crate::vault::FileFingerprint> = self.vault.notes.iter().filter_map(|note| self.vault.fingerprint(note.id).map(|fingerprint| (note.id, fingerprint))).collect();
        let (vault, catalog) = match crate::vault::Vault::scan(&notes_path) {
            Ok(result) => result,
            Err(error) => {
    ...
    fn build_tree_from_catalog(&mut self, entries: Vec<crate::vault::CatalogEntry>, depth: usize) -> Vec<FileTreeItem> {
        let mut items = Vec::new();
        for entry in entries {
            match entry {
                crate::vault::CatalogEntry::Folder(folder) => {
                    let folder = *folder;
                ...
                crate::vault::CatalogEntry::Note(note) => {
                    let note = *note;
    ...
    pub(crate) fn persist_active_body(&mut self, body: String) -> bool {
        ...
        if let Err(error) = crate::vault::save_note(&path, &body) {
    ...
    pub(super) fn update_wiki_links_after_moves(&self, moves: &[(String, String, String)]) -> Result<(), String> {
        ...
        for (applied, update) in updates.iter().enumerate() {
            if let Err(error) = crate::vault::save_note(&update.path, &update.modified) {
                let mut rollback_errors = Vec::new();
                for previous in updates[..applied].iter().rev() {
                    if let Err(rollback) = crate::vault::save_note(&previous.path, &previous.original) {
    ...
    pub(super) fn replace_wiki_links_in_content(&self, content: &str, moves: &[(String, String, String)]) -> String {
        ...
        let frontmatter_end = crate::core::markdown::frontmatter_end(content);
        crate::core::markdown::visit_document_wiki_links_with_tilde_fences(content, frontmatter_end, true, |located| {
...
mod tests {
    ...
    fn dialog_created_note_uses_the_root_template() {
        ...
        let (frontmatter, _) = crate::vault::Frontmatter::parse(&content);
