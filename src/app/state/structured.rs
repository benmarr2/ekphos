...
use crate::bases::{BaseFile, BaseRecord, CompiledBase, Corpus, Value};
use crate::canvas::{CanvasEdge, CanvasEnd, CanvasNode, CanvasNodeKind, CanvasSide};
...
impl App {
    pub fn active_document_kind(&self) -> Option<crate::vault::VaultFileKind> {
    ...
    pub(crate) fn refresh_structured_document(&mut self) {
        ...
        match kind {
            crate::vault::VaultFileKind::Markdown => {}
            crate::vault::VaultFileKind::Base => self.refresh_base(&source),
            crate::vault::VaultFileKind::Canvas => self.refresh_canvas(&source, reset_canvas_selection),
        }
    ...
    fn refresh_base(&mut self, source: &str) {
        ...
        let definition = match crate::bases::parse_base(source) {
    ...
    fn refresh_canvas(&mut self, source: &str, reset_selection: bool) {
        self.structured.base_worker.cancel();
        self.structured.base = BaseViewState::default();
        match crate::canvas::parse_canvas(source) {
    ...
    pub(crate) fn poll_base_evaluation(&mut self) -> bool {
        ...
        if response.generation != self.structured.base.request_generation || self.active_document_kind() != Some(crate::vault::VaultFileKind::Base) {
            return false;
        }
    ...
    fn push_canvas_undo(&mut self, previous: crate::canvas::Canvas) {
    ...
    fn persist_canvas_document(&mut self, success_message: &str) -> bool {
        let source = match self.structured.canvas.document.as_ref().map(crate::canvas::Canvas::to_json_pretty) {
    ...
    pub(crate) fn poll_structured_vault(&mut self) -> bool {
        if self.editor.mode != Mode::Normal || self.active_document_kind() != Some(crate::vault::VaultFileKind::Base) {
            return false;
        }
...
fn next_canvas_edge_id(document: &crate::canvas::Canvas) -> String {
    ...
    for note in notes {
        ...
        if note.kind.is_markdown() {
            if let Ok(body) = std::fs::read_to_string(path) {
                let (frontmatter, frontmatter_end) = crate::vault::Frontmatter::parse(&body);
                if let Some(frontmatter) = frontmatter {
                    ...
                    if let Some(date) = frontmatter.date {
                        properties.insert("date".to_string(), crate::bases::parse_date(&date).map(Value::Date).unwrap_or(Value::String(date)));
                    }
                ...
                properties.insert("tags".to_string(), Value::List(tags.iter().cloned().map(Value::String).collect()));
                links = crate::core::markdown::document_wiki_links_with_tilde_fences(&body, frontmatter_end.checked_sub(1), true).into_iter().map(|link| link.link.target.to_string()).collect();
            }
        }
...
fn vault_signature(root: &std::path::Path) -> u64 {
    fn visit(root: &std::path::Path, path: &std::path::Path, hasher: &mut std::collections::hash_map::DefaultHasher) {
        ...
        for entry in entries {
            ...
            } else if file_type.is_file() && crate::vault::VaultFileKind::from_path(&path).is_some() {
...
"#,
    ...
    fn canvas_file_nodes_open_catalog_documents() {
        ...
        assert_eq!(fixture.app.active_document_kind(), Some(crate::vault::VaultFileKind::Markdown));
    }
    ...
    fn canvas_edits_persist_positions_and_connections() {
        ...
        let (saved, diagnostics) = crate::canvas::parse_canvas(&saved).unwrap();
    ...
    fn canvas_resize_persists_dimensions_and_undo_restores_them() {
        ...
        let (saved, diagnostics) = crate::canvas::parse_canvas(&source).unwrap();
        assert!(diagnostics.is_empty());
