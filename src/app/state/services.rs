...
impl App {
    pub fn poll_background(&mut self) -> BackgroundActivity {
        ...
        let structured_work = self.editor.mode == Mode::Normal && self.active_document_kind() == Some(crate::vault::VaultFileKind::Base);
    ...
    pub(super) fn update_editor_wiki_links_with_ranges(&mut self, ranges: &[crate::editor::WikiLinkRange]) {
        ...
        let validated_ranges: Vec<crate::editor::WikiLinkRange> = ranges
            ...
            .map(|range| {
                ...
                crate::editor::WikiLinkRange { row: range.row, start_col: range.start_col, end_col: range.end_col, is_valid }
            })
            .collect();
    ...
    pub(super) fn validate_wiki_link_at(&self, row: usize, start_col: usize, valid_targets: &HashSet<String>) -> bool {
        ...
        let Some(link) = crate::core::markdown::wiki_link_at(line, byte_start) else {
            return false;
        };
