...
use crate::editor::{CursorShape, Editor, Position};
use crate::graph;
use crate::graph::{GraphEdge, GraphFileFingerprint, GraphFilter, GraphIndex, GraphLinkScope, GraphMode, GraphNode, GraphResponse, GraphSourceFile, GraphSourceMetadata, GraphWorker};
...
use crate::search;
use crate::search::{SearchHit, SearchIndex, SearchWorker};
...
fn heading_text(line: &str) -> Option<&str> {
    crate::core::markdown::heading(line.trim_start()).map(|heading| heading.text)
}
...
fn save_last_opened_note(cache_dir: &std::path::Path, path: &std::path::Path) {
    if fs::create_dir_all(cache_dir).is_ok() {
        let _ = crate::vault::save_note(&last_note_path(cache_dir), &path.to_string_lossy());
    }
}
