...
impl Editor {
    ...
    fn ensure_fold_projection(&self) {
        ...
        let frontmatter_end = ekphos_core::markdown::frontmatter_end_in_lines(self.buffer.iter_lines());
        ...
        for (row, line) in self.buffer.iter_lines().enumerate() {
            ...
            if fence.is_none() && marker.is_none() {
                if let Some(heading) = ekphos_core::markdown::heading(line) {
    ...
    pub(super) fn is_frontmatter_row(&self, row: usize) -> bool {
        ekphos_core::markdown::frontmatter_end_in_lines(self.buffer.iter_lines()).is_some_and(|end| row <= end)
    }
